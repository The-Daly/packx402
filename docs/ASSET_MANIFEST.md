...
**Torn-open variants: 4 of 14 tiers** have a real Higgsfield-generated torn-pack render at
`public/packs/{tierKey}-torn.png` — Spark, Scout, Obsidian, Mythic (the tiers used in the
landing-page demo). Generated by editing each tier's existing front-facing art with
Higgsfield's `nano_banana_pro` (image-reference edit), prompted to keep the exact same
character/palette/style but tear the top open with a jagged foil seam and a peeled/curling
corner. `PackArt`'s `torn` prop requests this path and falls back silently to the closed
pack art if the file is missing (same fallback pattern as everything else here) — so adding
the remaining 10 tiers' torn variants is a drop-in, zero-code-change follow-up.
`OpeningStage` shows the torn variant during the brief `"tearing"` phase, right after the
user's drag-to-rip gesture commits (see `RipToOpen.tsx`) and before the reveal wheel spins.
